...
namespace HX
{
	public class Emitter : MonoBehaviour
	{
		...
		public State state
		{
			...
			private set
			{
				...
				switch (state)
				{
					...
					case State.COOLING:
						shootIdx = 0;
						if (onCooldown != null) onCooldown(this);
						break;
				}
			}
		}
		...
		#region event
		public Action<Emitter, Projectile> onShoot;
		public Action<Emitter> onCooldown;
		#endregion
		...
		private void ShootProc()
		{
			...
			for (projectileIdx = 0; projectileIdx < _bundle && HasAmmo(); ++projectileIdx)
			{
				...
				if (onShoot != null)
					onShoot(this, _projectileGO.GetComponent<Projectile>());
			}
